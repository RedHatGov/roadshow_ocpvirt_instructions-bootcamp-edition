...
[NOTE]
====
Please ensure that the virtual machine named fedora02 has a label assigned "app=fedora02" and "vm.kubevirt.io/name=fedora02" to ensure that the restore function works. This is needed since the fedora02 machine was a clone of fedora01 and has the old labels from that virtual machine instance.
====
