...
[IMPORTANT]
Any CR that already exists is not modified or restored from a backup.  OADP manages this by attempting to create the resource and if the create fails, it moves on to the next resource.  Therefore you can backup all VMs in a single namespace and restore just one without impacting the others.
