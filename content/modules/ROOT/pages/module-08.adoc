...
. Scroll back to the left on the horizontal menu bar to select *BackupStorageLocation*.  Note the name is the same as the DPA with _-1_ appended to the end and is in an available state.
+
...
+
. In the Delete VirtualMachine pop-up, confirm the correct VM name is displayed and the _Delete Disks_ checkbox is selected.  Click *Delete*.
+
