...
+
. When host discovery and hardware inspection is complete you will see that it shows its status as *Available*.
+
...
+
. Once provisioning is complete you will see the node listed with its Phase set to *Provisioned as node*.
+
...
+
. We can also click directly on *worker4* under the *Name* column to see its current status.
+
