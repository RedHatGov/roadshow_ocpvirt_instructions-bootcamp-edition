...
+
.. Click *Project: All Projects* at the top left of the window and then click *Create Project*.
+
...
. In the top section on the right, you will see a collapsed area called *Optional parameters*. Expand that section and you will see where you can set the *CLOUD_USER_PASSWORD*.  Feel free to set this to something easy to remember/type so later in the course, you can easily login as the *fedora* user.  Click the *Quick create VirtualMachine* icon in the lower left to create the Fedora VM.
+
image::module-01/23_VM_Optional_Parameters.png[link=self, window=blank, width=100%]

. After a few seconds, expect to see the VM is *Running*. During this time, the storage provider has cloned the template disk so that it can be used by the newly created virtual machine. The amount of time this takes can vary based on the storage provider being used to create the boot disk. This is where VM deployment can benefit from using Ceph as the back end storage provider.
+
...
This concept will be a new concept for those coming from other virtualization platforms.  Those administrators are used to Virtual Machines being directly connected to Data Center VLAN networks. Openshift Virtualization does have this capability as well, utilizing xref:https://docs.openshift.com/container-platform/4.16/networking/multiple_networks/understanding-multiple-networks.html[Multus].
