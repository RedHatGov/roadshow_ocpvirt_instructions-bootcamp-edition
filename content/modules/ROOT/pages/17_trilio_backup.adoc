...
+
image::Trilio/openshiftlogin.PNG[link=self, window=blank, width=100%]
...
+
. On the Create Target pop-up window, enter the following information:
* Namespace
* Threshold Capacity
* Vendor Details - select ObjectStore 
* Vendor - Other
* Bucket Name - from the OCP GUI, select *Storage*, *Object Storage*, *ObjectBucketClaims*, select the *trilio-bucket*.  Scroll to the bottom and use the copy features for the _Bucket Name_ field and paste it to the _Bucket Name_ field in the _Create Target_ field.
* Region - us-east-1
...
* Object store credentials - from the OCP GUI, select Storage, Object Storage, ObjectBucketClaims, select the trilio-bucket.  Scroll to the bottom and use the copy features for the AWS Access Key and Secret Key to copy and paste them in the appropriate fields on the Create Secret dialog window.
