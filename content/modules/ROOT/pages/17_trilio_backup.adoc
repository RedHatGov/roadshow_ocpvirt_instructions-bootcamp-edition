...
. In the left pane, expand *Operators*, select *Operator Hub*.  In the search box, enter *Trilio*.  Click on the *Trilio* tile.  
+
image::Trilio/operator.png[link=self, window=blank, width=100%]
+
. Click the *Install* icon on the _Trilio for Kubernetes_ pop-up.  
+
image::Trilio/operator2.png[link=self, window=blank, width=100%]
+
. Keep all the default options. The operator will be installed in the _trilio-system_ namespace.  All pods related to Trilio will run in this namespace as to not interfere with security restrictions and quota restrictions. To enable the _Trilio Backups_ menu options in the left navigation pane in the OpenShift console, select *Enable* under _Console Plugin_. 
+
[NOTE]
...
+  
. Once the operator is installed, click on *Refresh Web* link.  This will add the _Trilio Backups_ plug-in to the OpenShift Console GUI. After the console refreshes, click the *View Operator* icon.
+  
image::Trilio/operator4.png[link=self, window=blank, width=100%]
+  
. On the Details pane, click *Create instance* link, this will create an instance of Trilio and start all the required pods.  
+  
image::Trilio/operator5.PNG[link=self, window=blank, width=100%]
+  
. On the _Create TrilioVaultManager_ page, for this demo, you can accept the defaults.  Scroll to the bottom, click the *Create* icon.  
[NOTE]
For customer environments, some of the defaults may need to be changed.  There are various logLevel that could provide a deeper level of information for troubleshooting.  Affinity rules and tolerations can also be configured along with CPU and memory requests and limits.  
+  
...
+  
image::Trilio/operator7.png[link=self, window=blank, width=100%]
...
+
image::Trilio/triliobucket.png[link=self, window=blank, width=100%]
...
* Threshold Capacity: 1 TiB
* Vendor Details: ObjectStore 
* Vendor: Other
...
* Object store credentials: from the _OpenShift Console GUI_, select *Storage*, *Object Storage*, *ObjectBucketClaims*, select the *trilio-bucket*.  Scroll to the bottom and use the copy features for the _Access Key_ and _Secret Key_ to copy and paste them in the appropriate fields in the _Create Secret_ dialog window.
...
+
. The target will initially be in an `InProgress` state.  After a few minutes, the target should be in an `Available` state.  You may need to scroll to the right to see the _Status_ field.
+
...
In the _Virtual Machine Management_ lab exercise, we had created the _vmexamples_ namespace along with virtual machine fedora-01.  In the _Storage Management_ lab we created the fedora-02 virtual machine.  And if you completed the _Migrating Virtual Machines_ lab, you also will have the winweb0[12] and database virtual machines. For the _Trilio Data Protection_ lab, please apply all instructions to the _vmexamples_ namespace.

To obtain a consistent snapshot of the virtual machines, Trilio uses the virt-freeze/unfreeze features of the qemu-guest-agent.  This will quiesce the filesystem and flush the buffers prior to taking the snaphot and then unfreeze the filesystems after the snapshot has completed.  
[NOTE]
For more information on the quiesce features, see https://github.com/kubevirt/kubevirt/blob/main/docs/freeze.md. 

. From the left navigation pane, expand *Trilio Backup*, select *Backups*.  Click the *Create Backup* icon.

image::Trilio/createbackup.PNG[]
