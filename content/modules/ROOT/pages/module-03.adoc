...
== Attach an additional disk to a virtual machine

An additional disk may be required for an application that runs on the virtual machine to keep data separate from the root filesystem.  You can easily attach disks to a virtual machine while it is running.
...
+
. Click the *Add Disk* icon.  In the _Add Disk_ dialog window, update the name to *appdisk*, and enter 5 GiB for the _PersistentVolumeDisk size_.  Click the *Save* icon.  The PVC and underlying PV will be created and attached to the VM as /dev/sda in this case.  
+
...
+
image::module-03/22_VM_consolesda.png[link=self, window=blank,width=100%]
+
[NOTE]
You will also the console output from the disk discovery in the console.  You may have to hit the *Enter* key to get the login prompt.
...
sudo mkdir /app
sudo echo /dev/sda /app ext4 defaults 0 0 >> /etc/fstab
