...
+
image::module-02/01_Compute_Nodes.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/02_Worker0_Information.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/03_Worker0_Details.png[link=self, window=blank, width=100%]

NOTE: On the *Details* page you will notice the *Model* of the server is _Red Hat, KVM (8.6.0)_ for each of the nodes.  This is because in order to reduce costs, this lab is 7 Virtual Machines deployed on a single bare metal host.  The breakdown of the actual deployment topology is described in the Networking section of the Troubleshooting Chapter.
...
+
image::module-02/04_BMHosts.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/05_Worker0_BMHost.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/06_Add_Host_Red.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/07_Create_Host_Red.png[link=self, window=blank, width=100%]
NOTE: If your worker4 gets stuck in _Powering On_, ensure you are creating worker4 in the *openshift-machine-api* project.  If you have confirmed that, and it is still not working, on the hypervisor node, restart the Virtual BMC by executing:
`sudo systemctl restart virtualbmc.service`
+
...
+
image::module-02/08_Worker4_Summary_1.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/09_Worker4_Summary_2.png[link=self, window=blank, width=100%]
+
...
----
+
image::module-02/09a_Worker4_Yaml_Edit.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/14_Worker_4_Provisioning.png[link=self, window=blank, width=100%]
+
. Once provisioning is complete you will see the node listed with it's Phase set to *Provisioned as node*.
+
image::module-02/15_Provisioned_As_Node.png[link=self, window=blank, width=100%]
+
...
+
image::module-02/16_All_Nodes.png[link=self, window=blank, width=100%]
+
. We can also click directly on *worker4* under the *Name* column to see it's current status.
+
image::module-02/17_Worker_4_Details.png[link=self, window=blank, width=100%]
+
